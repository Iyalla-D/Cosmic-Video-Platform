...
export default function Earth() {
    ...
    const handleMouseEvent = {
        down: (event) => {
            ...
            if (intersect?.uv) {
                const segment = randomizedSegmentsRef.current.find(
                    (s) => intersect.uv.y > s.minY && intersect.uv.y <= s.maxY,
                );
        ...
        up: (event) => {
            ...
            if (intersect?.uv) {
                ...
                if (segment?.id === mouseDownSegmentRef.current) {
                    const subsectionIndex = Math.floor(
                        intersect.uv.x * SUBSECTIONS_PER_SEGMENT,
                    );
        ...
        move: (event) => {
            ...
            if (intersect?.uv && randomizedSegmentsRef.current) {
                ...
                if (segment) {
                    ...
                    const subsectionVideo = segment.videos[subsectionIndex];
                    handleMouseEvent.handleVideoHover(
                        subsectionVideo,
                        tooltip,
                        event,
                    );
        ...
        resetTooltip: (tooltip) => {
            ...
            videoElementCache.current.forEach(({ videoElement }) =>
                videoElement.pause(),
            );
        },
    };
            ...
            try {
                ...
                const segments = Array.from(
                    ...
                    (_, segmentIdx) => ({
                        ...
                        videos: Array.from(
                            { length: SUBSECTIONS_PER_SEGMENT },
                            () => ({
                ...
                shuffledPositions
                    ...
                    .forEach((position, videoIndex) => {
                        const segmentIndex = Math.floor(
                            position / SUBSECTIONS_PER_SEGMENT,
                        );
    ...
    const videoMaterial = useMemo(() => {
        ...
        const uniforms = videoTextures.reduce(
            (acc, { texture }, idx) => ({
                ...acc,
        ...
        return new THREE.ShaderMaterial({
            ...
            fragmentShader: createFragmentShader(
                videoTextures,
                randomizedSegmentsRef.current,
                SUBSECTIONS_PER_SEGMENT,
            ),
